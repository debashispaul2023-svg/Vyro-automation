...
def _get_video_metadata(video_path: str) -> Tuple[float, int, int]:
    """Cache expensive video metadata (duration, width, height) to avoid redundant parsing."""
    clip = None
    try:
...
def _check_duration(video_path: str, req: CampaignRequirements) -> Optional[str]:
    ...
    try:
        duration, _, _ = _get_video_metadata(video_path)
...
def _check_resolution(video_path: str) -> Optional[str]:
    ...
    try:
        _, width, height = _get_video_metadata(video_path)
    except Exception as exc:  # noqa: BLE001
