...
def run_pre_upload_checks(
    ...
    # ⚡ Bolt: Read video metadata once to avoid multiple expensive ffmpeg subprocess spawns.
    duration, width, height = 0.0, 0, 0
    video_read_error: Optional[str] = None
    if VideoFileClip is None:
        video_read_error = (
            "moviepy is not installed; cannot verify video duration or resolution."
        )
    elif not os.path.isfile(video_path):
        video_read_error = f"Video file not found for checks: {video_path}"
    else:
        clip = None
        try:
            clip = VideoFileClip(video_path)
            duration, width, height = clip.duration, clip.w, clip.h
        except Exception as exc:  # noqa: BLE001
            video_read_error = f"Failed to read video file: {exc}"
        finally:
            if clip is not None:
                clip.close()
    ...
    _run(
        ...
        lambda: video_read_error or _check_duration(duration, req),
    )
    _run(
        "resolution_9x16", lambda: video_read_error or _check_resolution(width, height)
    )
