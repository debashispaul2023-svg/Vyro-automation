...
def run_pre_upload_checks(
    ...
    if VideoFileClip is None:
        _run("duration_within_range", lambda: "moviepy is not installed; cannot verify video duration.")
        _run("resolution_9x16", lambda: "moviepy is not installed; cannot verify video resolution.")
    elif not os.path.isfile(video_path):
        _run("duration_within_range", lambda: f"Video file not found for duration check: {video_path}")
        _run("resolution_9x16", lambda: f"Video file not found for resolution check: {video_path}")
    else:
        clip = None
        duration = None
        width = None
        ...
        error_msg = None
        try:
            # ⚡ Bolt: Cache VideoFileClip properties to avoid redundant expensive ffprobe subprocess calls.
            ...
            error_msg = f"Failed to read video metadata: {exc}"
        finally:
            if clip is not None:
                clip.close()

        if error_msg:
            _run("duration_within_range", lambda: error_msg)
            _run("resolution_9x16", lambda: error_msg)
        else:
