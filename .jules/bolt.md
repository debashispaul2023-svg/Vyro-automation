...
## 2026-08-27 - VideoFileClip Redundant Opening
**Learning:** Instantiating `moviepy.editor.VideoFileClip` is expensive due to underlying `ffprobe` subprocess calls. Opening the same file multiple times (e.g., to check duration and then to check resolution) causes redundant parsing overhead.
**Action:** Cache the properties (e.g., duration, w, h) of a video file when they are first read to prevent redundant opening of the same file.