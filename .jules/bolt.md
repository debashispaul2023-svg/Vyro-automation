...
## 2025-02-18 - MoviePy VideoFileClip Initialization Overhead
**Learning:** Instantiating `VideoFileClip` in MoviePy is a slow, synchronous operation that blocks the main thread while it probes the video file with ffmpeg to extract metadata (duration, resolution, fps). Opening the same file multiple times in different parts of the validation pipeline significantly increases latency (~90-100ms per initialization).
**Action:** Always extract and cache all necessary metadata (duration, width, height) immediately upon the first instantiation of a `VideoFileClip`, and pass the cached values down to subsequent functions instead of passing the filepath and reopening the file.
